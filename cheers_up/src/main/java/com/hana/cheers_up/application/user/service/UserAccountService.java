...
public class UserAccountService {
    ...
    @Transactional
    public UserAccountDto searchUserOrSave(String username, KakaoOAuth2Response kakaoResponse) {
        return userAccountRepository.findById(username).map(UserAccountDto::from)
                .orElseGet(() -> {
                    UserAccount saved = userAccountRepository.save(
                            UserAccount.of(username, kakaoResponse.email(), kakaoResponse.nickname(), null, RoleType.USER)
                    );
